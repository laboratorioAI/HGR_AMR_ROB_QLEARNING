function weights = trainQNN_Exp_Replay(numNeuronsLayers, transferFunctions, options, typeWorld, flagDisplayWorld)
...
%-------------Feb--------------------
num_prev_actions = evalin('base', 'num_prev_actions');
loops = evalin('base', 'num_replays');
Ltot=1;
numTestingSamples = max(0.1*40+num_prev_actions*6, 20);
testStates = zeros(numTestingSamples, 40+num_prev_actions*6); 
averageMaxQvalue = zeros(1, numEpochs*loops);
historyAverageWins = zeros(1, numEpochs*loops);
%------------------------------------------

% Gamma
gamma = options.gamma;                                                   % options.gamma = 1; % Q-learning parameter
% Training the ANN
cost = zeros(1, numEpochs*loops);                                              % vector de zeros 'cost' de dimension  options.numEpochs = 100000;
% Initializing the history of the average reward
averageEpochReward = zeros(1, numEpochs*loops);                                % vector de zeros 'averageEpochReward' de dimension  options.numEpochs = 100000;
...
%Inicializo 'averageMaxQvalue' vector de zeros (1 val x epoch)
...
s  =  nan(lengthBuffer, 40+num_prev_actions*6);
sp =  nan(lengthBuffer, 40+num_prev_actions*6);
...
for epoch = 1:numEpochs*loops %numero total de muestras de todos los usuarios
    
    %-------feb-----------------
    epoch_counter=epoch_counter+1;
    if epoch_counter==numEpochs
    epoch_counter=0;
    Ltot=Ltot+1;
    Code_0(rangeDown);
    else
    ...
    for k3 = 2:Numero_Ventanas_GT   %aqui va 2 ya que necesito el ground truth desde la segunda ventana
...
%             %disp('idxRand');disp(idx(1));
        ...
        new_state = table2array(Features_GT); %feature vector de nuevo estado
        
        %--------------feb--------------
        %action_one_hot=one_hot_action_func(action);
        %Por una sola vez, completo el vector 'new_state' con el numero de
        %ceros (6 x cada acción) necesaro dependiendo las acciones seleccionadas
        if numIteration==1
        ...
        etiqueta_actual=etiquetas(numIteration,1);     % ground truth de cada ventana EMG - CAMBIAR - etiqueta_actual=etiquetas(numIteration,1); 
       ...
       if  acciones_predichas_actual ~= etiqueta_actual  %conteo de clasificaciones exitosas
        ...
        %gameReplay(idx, :) = [state, action, reward, new_state];
        gameReplay(idx, :) = [state, action, reward, new_state_full];   %[state(:)', action, reward, new_state(:)'];
        %---------------------------------------------------------------
...
%             end
        ...
        % Updating the state
        state = new_state_full; %state = new_state
    ...
    fprintf('Epoch: %d of %d, cost = %3.2f, result = %s, epsilon = %1.2f, Qval = [%3.2f, %3.2f, %3.2f, %3.2f, %3.2f, %3.2f] \n',...
        epoch, numEpochs*loops, cost(epoch), resultGame, epsilon, Qval(1), Qval(2), Qval(3), Qval(4), Qval(5), Qval(6));
